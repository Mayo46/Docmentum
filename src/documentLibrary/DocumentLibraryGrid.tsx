...
import type { ICellRendererParams, ColDef, ValueGetterParams } from 'ag-grid-community'
...
function getCellValue(row: DocumentLibraryItemRow | undefined, columnKey: string) {
  if (!row) return ''
  ...
  if (key === 'createdby' || key === 'created by') return row.createdByDisplayName ?? ''
  if (key === 'modifiedby' || key === 'modified by' || key === 'last modified') return row.modifiedByDisplayName ?? ''
...
export default function DocumentLibraryGrid(props: Props) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const selectedKeys = Array.from(
        new Set([...columns.map((c) => c.key), documentClientUrlFieldKey]),
      )
  ...
  }, [client, parentDriveItemId, columns, documentClientUrlFieldKey])
  ...
  const columnDefs: ColDef<DocumentLibraryItemRow>[] = useMemo(() => {
    const defs: ColDef<DocumentLibraryItemRow>[] = columns.map((c) => ({
      ...
      valueFormatter: (params) => {
        ...
        if (kind === 'number') return params.value === '' || params.value === null || params.value === undefined ? '' : String(params.value)
  ...
  return (
    <Box>
      ...
      <UploadDialog
        ...
        onUpload={async ({ files, contentType, properties }) => {
          ...
          if (result.failures.length > 0) {
            ...
            setToast({ kind: 'success', message: `Uploaded ${files.length} file(s).` })
          }
      ...
      <Dialog open={deleteOpen} onClose={() => setDeleteOpen(false)}>
        ...
        <DialogContent>
          <Typography>
            {deleteTarget ? `Are you sure you want to delete "${deleteTarget.name}"?` : 'Are you sure?'}
          </Typography>
        </DialogContent>
        <DialogActions>
