...
type Props = {
  ...
  /** Drive folder id to open initially (e.g. document set). Empty string = library root. */
  parentDriveItemId: string;
  /** Label for the drive root crumb (e.g. library name). */
  libraryRootLabel?: string;
  /** Display name for the initial `parentDriveItemId` segment when it is set. */
...
function formatContentTypeValue(raw: unknown): string {
  ...
  if (typeof raw === "string") {
    const s = raw.trim();
    if (!s) return "";
    // Hex content type id — not a display name; leave empty for graphClient to fill via contentTypeName
  ...
  if (typeof raw === "object") {
    ...
    const name =
      o.name ?? o.label ?? o.displayName ?? o.Title ?? o.title;
            ...
            return (
              ...
              >
                ...
                <Link
                  ...
                  aria-label={`Version history for ${label}`}
