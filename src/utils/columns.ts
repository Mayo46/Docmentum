...
export function getCellValue(
  ...
  if (key === "modified")
    return getFieldValueCaseInsensitive(row.fields, "Modified") ?? "";
  if (key === "created")
    return getFieldValueCaseInsensitive(row.fields, "Created") ?? "";
  if (key === "createdby") return row.createdByDisplayName ?? "";
  if (key === "modifiedby") return row.modifiedByDisplayName ?? "";
  return (
    row.fields?.[columnKey] ??
...
export function normalizeColumnsInput(columns?: unknown): string[] {
  ...
  if (typeof columns === "string") {
    ...
    return uniqueNonEmpty(raw.split(",").map((c) => toCanonicalKey(c)));
  }

  if (typeof columns === "object") {
    return uniqueNonEmpty(
      parseColumnsObject(columns as Record<string, unknown>),
    );
  }
