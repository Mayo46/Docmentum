...
export function useDocumentLibraryNavigation({
  ...
  const uploadsEnabled = Boolean(currentParentDriveItemId);
  ...
  const navigateToParent = useCallback((parent: ParentReference) => {
    if (!parent.itemId || !parent.name) return;
    setSegments((prev) => {
      const existingParentIndex = prev.findIndex(
        (segment) => segment.id === parent.itemId,
      );

      if (existingParentIndex >= 0) {
        return prev.slice(0, existingParentIndex + 1);
      }
